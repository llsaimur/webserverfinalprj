...
if (!isset($_POST['username'], $_POST['password'])) {
    header("Location: login.php?error=Please%20fill%20in%20both%20username%20and%20password");
    exit;
}
...
if ($result->num_rows == 1) {
    ...
    if (password_verify($_POST['password'], $row['passCode'])) {
        ...
        if (isset($_SESSION['registrationOrder'])) {
            header("Location: ../../levels/level1.php"); // Redirect to level 1
            exit;
        } else {
            // If registration order is not set, redirect back to login with an error message
            header("Location: login.php?error=Sorry,%20an%20error%20occurred.%20Please%20try%20logging%20in%20again.");
            exit;
        }
    } else {
        // Incorrect password
        header("Location: login.php?error=Sorry,%20the%20username%20or%20password%20is%20incorrect!");
        exit;
    }
} else {
